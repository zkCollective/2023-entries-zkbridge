...
import {Header} from "./Updater.sol";

library SSZ {


	function sszBeaconBlockHeader(Header memory header) internal pure returns (bytes32) {
    ...
    function computeSigningRoot(Header memory header, bytes4 forkVersion, bytes32 genesisValidatorsRoot) internal view returns (bytes32) {
