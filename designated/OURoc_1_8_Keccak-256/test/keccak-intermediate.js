...
describe("Keccak-FinalV test", function () {
    this.timeout(100000);

    let cir;
    before(async () => {
	cir = await wasm_tester(path.join(__dirname, "circuits", "finalv_test.circom"));
	await cir.loadConstraints();
	console.log("n_constraints", cir.constraints.length);
    });
    it ("Final 1 (testvector generated from go)", async () => {
	...
	const inIn = utils.bytesToBits(padInput(input));
	const expectedOutBits = utils.u64ArrayToBits(expectedOut);

	const witness = await cir.calculateWitness({ "in": inIn, "len": len }, true);
