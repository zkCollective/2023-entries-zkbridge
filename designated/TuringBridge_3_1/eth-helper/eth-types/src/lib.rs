...
arr_ethereum_types_wrapper_impl_serde_ssz!(H64, 8);
arr_ethereum_types_wrapper_impl_serde_ssz!(H128, 16);
arr_ethereum_types_wrapper_impl_serde_ssz!(H160, 20);
arr_ethereum_types_wrapper_impl_serde_ssz!(H256, 32);
arr_ethereum_types_wrapper_impl_serde_ssz!(H512, 64);
arr_ethereum_types_wrapper_impl_serde_ssz!(H520, 65);
arr_ethereum_types_wrapper_impl_serde_ssz!(Bloom, 256);
...
/* impl RlpDecodable for BlockHeader {
    fn decode(serialized: &Rlp) -> Result<Self, RlpDecoderError> {
        ...
        /*         block_header.hash = Some(
            near_keccak256({
                let mut stream: RlpStream = RlpStream::new();
